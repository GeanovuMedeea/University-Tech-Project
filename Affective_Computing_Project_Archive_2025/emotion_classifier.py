...
# Load the trained SVM model and the scaler, no training again
best_model = joblib.load('svm_emotion_model.pkl')
scaler = joblib.load('scaler.pkl')
emotion_labels = ['anger', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral', 'unknown']

# Load emotion labels from the CSV file
label_csv = pd.read_csv('video_content_labels.csv')
label_dict = dict(zip(label_csv['second'], label_csv['label']))

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

def combine_predictions(svm_probs, cnn_probs, true_label=None):
    combined_probs = {}

    #print('t',true_label)
    if true_label:
        for emotion in svm_probs:
            combined_probs[emotion] = 0.4 * svm_probs[emotion] + 0.4 * cnn_probs.get(emotion, 0)

            if emotion == true_label:
                combined_probs[emotion] += 0.2

    else:
        # Default behavior if no label is provided
        for emotion in svm_probs:
            combined_probs[emotion] = 0.5 * svm_probs[emotion] + 0.5 * cnn_probs.get(emotion, 0)
...
def monitor_csv(csv_file):
    ...
    while True:
        ...
        if current_time - last_checked_time > 5:  # Check every 5 seconds
            try:
...
def process_new_data(csv_file, model, new_rows=None, batch_size=1):
    ...
    # Save global indices for new rows
    if new_rows:
        global_indices = list(new_rows)  # Keep the original CSV row indices
        dataset.data = dataset.data.iloc[global_indices]
    else:
        global_indices = list(dataset.data.index)  # Use all indices if no specific new_rows
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(data_loader):
            ...
            # Flatten the input landmarks to a 2D array (samples, features)
            input_array = inputs.view(inputs.size(0), -1).cpu().numpy()  # Flatten array
            ...
            for i, global_idx in enumerate(batch_indices):
                ...
                # Retrieve the second value from landmarks_data.csv
                second_value = df.loc[global_idx, 'pth']
                second = int(second_value.split('_')[-1].split('.')[0])  # Extract the second from filename (ex. 'frame_003.jpg' -> 3)

                # Retrieve the true label from label_dict based on the 'second' value
                true_label = label_dict.get(second)
                ...
                # Now the input is reshaped to 2D, so it can be passed to the scaler
                svm_predictions = best_model.decision_function(scaler.transform(input_array))  # Decision function to probability later

                # Apply softmax to get approximate probabilities
                svm_probs = softmax(svm_predictions, axis=1)[0]

                # Map decision function to emotions
                svm_probs_dict = {emotion_labels[i]: svm_probs[i] for i in range(len(emotion_labels))}

                # Combine the predictions from SVM and CNN, considering the true label
                combined_probs = combine_predictions(svm_probs_dict, cnn_probs, true_label=true_label)
                print(f"Combined Probabilities for row {global_idx}: {combined_probs}")

                # Select the final emotion based on combined probabilities
                final_emotion = select_final_emotion(combined_probs)

                # Update the corresponding row in the original CSV
            ...
            # Save changes back to the CSV after processing the batch

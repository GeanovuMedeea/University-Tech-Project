...
public class Controller {
    ...
    public void executeOneStep() throws IOException, InterruptedException, ToyLanguageInterpreterException {
        executor = Executors.newFixedThreadPool(2);
        MyIList<ProgramState> prgList = removeCompletedPrg(repository.getProgramStateList());
        if(prgList.getListSize().getVal()>0) {
            ProgramState program = prgList.get(0);
            program.getHeap().setContent(
                    garbageCollector(getAddrFromSymTable(program.getTopSymTable().getValues(),program.getHeap().getContent()), program.getHeap().getContent()));
        ...
        //MyIList<ProgramState> programStates = repository.getProgramStateList();
        //if(programStates.getListSize().getVal() > 0)
        //{
        //    try {
        //        oneStepForAllPrg(repository.getProgramStateList());
        //    } catch (InterruptedException | IOException | ToyLanguageInterpreterException e) {
        //        System.out.println();
        //    }
    ...
    private void printThings(){
        ...
        System.out.print("------------------------------------------------------\n");
        System.out.println("Thread number id: " + programState.getId().toString() + "\n");
    ...
    /*private void oneStepForAllProgram(MyList<ProgramState> states) throws InterruptedException {
        ...
        GUI.List<Callable<ProgramState>> callList = states.stream().filter(p->!p.getExecutionStack().isEmpty().getVal())
                .map((ProgramState p) ->
        ...
        GUI.List<ProgramState> newProgramList = executor.invokeAll(callList).stream()
                .map(future -> {
                    try {
                        return future.get();
                    } catch (InterruptedException | ExecutionException e) {
    ...
    public void allSteps() throws InterruptedException, ToyLanguageInterpreterException, IOException {
        runTypeChecker();
        executor = Executors.newFixedThreadPool(2);
        GUI.List<ProgramState> states = removeCompletedProgram(repository.getProgramStateList());
        // GUI.List<ProgramState> prgList = null;
        while(states.size() > 0){
            try {
                runTypeChecker();
            }

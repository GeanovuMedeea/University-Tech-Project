...
class UI:
    ...
    def floyd_warshall(self):
        ...
        while continue_program != 0:
            ...
            if dist[start_vertex][ending_vertex] == infinity:
                print("There is no lowest-cost walk between the vertices", start_vertex, "and", ending_vertex)
            else:
                # creates the lowest-cost walk between two vertices in the matrix by using the matrix with
                # previously parsed vertices -> starts from ending vertex and stops at start vertex. All the needed
                # vertices for the path are found on the line defined by the start vertex as line index
                print("The lowest-cost walk between the vertices", start_vertex, "and", ending_vertex, "is:",
                      dist[start_vertex][ending_vertex])
                path = []
    ...
    def start(self):
        ...
        commands_list_for_listing = {"1": self.print_cost_of_edge, "2": self.in_degree, "3": self.out_degree,
                                     ...
                                     "11": self.bfs_algorithm, "12": self.floyd_warshall}
